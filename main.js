...
function previewPDF() {
    ...
    readyPdfName = '請求書_' + clientName + '.pdf';
    const invoiceElement = document.getElementById('invoice-layout');
    
    const shareBtn = document.querySelector('.share-btn');
    shareBtn.innerText = "PDF準備中...";
    shareBtn.disabled = true;
    shareBtn.style.opacity = "0.5";

    setTimeout(() => {
        const opt = { margin: 0, filename: readyPdfName, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, scrollX: 0, scrollY: 0 }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } };
        html2pdf().set(opt).from(invoiceElement).output('blob').then(function(blob) {
            readyPdfBlob = blob;
            shareBtn.innerText = "送信・保存する ▶";
...
function executePDF() {
    ...
    const file = new File([readyPdfBlob], readyPdfName, { type: 'application/pdf' });
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        reader.readAsDataURL(readyPdfBlob);
        reader.onloadend = function() {
            const base64data = reader.result;
            const a = document.createElement('a');
            a.href = base64data;
            a.download = readyPdfName;
